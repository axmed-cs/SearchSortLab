...
public class BinarySearchTree {
    ...
    public BSTNode contains(BSTNode root, String needle) {
        ...
        if(root == null){
            ...
            return root;      // if needle is the root, return root
        }
        else if (needle.compareTo(root.getData()) < 0){
            return contains(root.getLeft(), needle);  // if needle is less than root
                                                      // return left side of root (recursion)
        }
        else{
            return contains(root.getRight(), needle);   //else needle is greater than root
                                                       // return right side of root (recursion)
        }
    }
    ...
    public BSTNode fromArray(String[] array, int start, int end) {
        ...
        if (start > end) {
            ...
            int half = (start+end)/2;                 //sets half at the middle of the array
            BSTNode root = new BSTNode(array[half]);  // creating root node at half

            root.setRight(fromArray(array, half+1, end));  //recursion towards right side of half (middle)
            root.setLeft(fromArray(array,start, half-1));   //recursion towards left side of half (middle)
